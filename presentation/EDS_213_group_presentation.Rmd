...
ow much time is needed to manage the data and who will be responsible for doing so?

Time: ~25% of the total project time for managing  data 
 - making sure  data is clean and accessible
 - uploading relevant files to a repository
 - making sure all individual data sets can be used together
Team: all team members responsible for these tasks
...
library(leaflet)
leaflet() %>% addTiles() %>% setView(-120, 34, zoom = 9)
```
...
Where?
- The data will be preserved on DataONE 

Why?
- DataOne is a more robust site that hosts a greater number of repositories when compared to other data repository sites like knb

How?
- relevant data will be saved as both a **csv** and **txt file** 
